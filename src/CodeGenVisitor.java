...
public class CodeGenVisitor extends ExprBaseVisitor<String> {
    ...
    private boolean isTopLevelLambda(String name) {
        return topLevelLambdas.containsKey(name);
    }

    private boolean isTopLevelMethod(String name) {
        return "method".equals(topLevelLambdas.get(name));
    }

    private boolean isTopLevelField(String name) {
        return "lambdaField".equals(topLevelLambdas.get(name));
    }

    private void registerTopLevelLambda(String name, String kind) {
        topLevelLambdas.put(name, kind);
    }
    ...
    private boolean isRecursiveLambda(String varName, ExprParser.LambdaExpressionContext lambdaCtx) {
        ...
        return bodyText.contains(varName + "(") || bodyText.contains(varName + " ("); // Detectar llamada recursiva (con
                                                                                      // o sin espacio)
    }
    ...
    public String visitLetStatement(ExprParser.LetStatementContext ctx) {
        ...
        // Si el tipo declarado es Double y el valor es un literal entero, corrige el
        // literal
        if ("Double".equals(type) && value.matches("^[0-9]+$")) {
            value = value + ".0";
        }
    ...
    public String visitPrimaryExpression(ExprParser.PrimaryExpressionContext ctx) {
        ...
        if (ctx.INTEGER() != null) {
            // Si el número está asignado a un Double, usa "1.0" para evitar cast
            String literal = ctx.INTEGER().getText();
            String parentText = ctx.getParent().getText();
            if (parentText.contains(":double") || parentText.contains("Double"))
                return literal + ".0";
            return literal;
        }
    ...
    public String visitCastExpression(ExprParser.CastExpressionContext ctx) {
        ...
        switch (targetType) {
            case "int" -> {
                ...
                if (expr.startsWith("\"") || expr.contains("String.valueOf"))
    ...
    private String mapType(ExprParser.TypeContext ctx) {
        ...
        // Si no hay flecha, es tipo simple
        if (!text.contains("->")) {
            return mapTypeName(text);
        }

        // Asegurarse de NO eliminar paréntesis incorrectamente
        String inside = text;
        if (inside.startsWith("(") && inside.endsWith(")")) {
            inside = inside.substring(1, inside.length() - 1);
        }

        // Dividir parámetros y retorno
        String[] parts = inside.split("->");
        if (parts.length != 2) {
            return "Function<Object, Object>"; // fallback seguro
        }

        String paramPart = parts[0].trim().replace("(", "").replace(")", "");
        String retPart = parts[1].trim();

        String[] paramTypes = paramPart.isEmpty() ? new String[0] : paramPart.split(",");
        List<String> mappedParams = new ArrayList<>();
        for (String p : paramTypes) {
            mappedParams.add(mapTypeName(p.trim()));
        }

        String mappedReturn = mapTypeName(retPart);

        return switch (mappedParams.size()) {
            case 0 -> "Supplier<" + mappedReturn + ">";
            case 1 -> "Function<" + mappedParams.get(0) + ", " + mappedReturn + ">";
            case 2 -> "BiFunction<" + mappedParams.get(0) + ", " + mappedParams.get(1) + ", " + mappedReturn + ">";
            default -> "Function<Object, " + mappedReturn + ">";
        };
    }
    ...
    private String mapTypeName(String t) {
        ...
        return switch (t) {
            ...
            case "any" -> "Object";
            case "void" -> "Void";
            default -> capitalize(t); // para tipos definidos con data
        };
    }
    ...
    private String inferType(ExprParser.ExpressionContext expr) {
        ...
        if (expr.lambdaExpression() != null) {
            ...
            if (lambda.lambdaParams().typedLambdaParams() != null) {
                List<ExprParser.TypedParamContext> typedParams = lambda.lambdaParams().typedLambdaParams().typedParam();
                ...
                if (paramTypes.size() == 1) {
                    ...
                    if (returnsVoid)
                        ...
                        return String.format("BiFunction<%s, %s, %s>", paramTypes.get(0), paramTypes.get(1),
                                mapTypeName(returnType));
                } else {
                    return returnsVoid
                            ? "Runnable"
    ...
    private Map<String, String> parseLambdaParamTypes(ExprParser.TypeContext typeCtx) {
        ...
        // solo devolvemos la lista de tipos; nombres los obtendremos en
